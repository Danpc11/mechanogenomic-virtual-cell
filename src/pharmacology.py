...
# ---------------------------------------------------------------------------
def hepatocyte_function(E, ph=None, sigma_half=25.0, hill=2.0, reps=6,
                        use_fast=True):
    """Hepatocyte functional index in [0, 1] as a function of the nuclear
    mechanical drive sigma (which itself depends on E AND the phenotype).

    1.0 = fully differentiated/functional (low drive, soft/healthy);
    ->0 = dedifferentiated/dysfunctional (high drive, stiff/fibrotic).
    Using sigma (not E alone) makes the index respond to drugs that act on the
    INTERNAL mechanics (ROCK/myosin -> nm, FAK/integrin -> nc), not only to
    drugs that lower external tissue stiffness. Consistent with reported loss
    of albumin/urea/CYP450/HNF4a as mechanotransduction rises on stiff matrices."""
    if ph is None:
        ph = PHENOTYPES["hepatocyte"]
    sig = _stress(E, ph, reps, use_fast)
    f = 1.0 / (1.0 + (sig / sigma_half) ** hill)
...
def toxicity_flag(drug, ph=None, E=None, reps=6):
    ...
    if E is None:
        E = FIBROSIS_STIFFNESS["F4"]
    # baseline function at current mechanical state
    f_base = hepatocyte_function(E, ph, use_fast=False)
    # treated: apply BOTH the external stiffness change (E_mod) AND the internal
    # mechanical perturbations (nm, nc, laminAC) so ROCK/FAK/integrin drugs count
    ph_t = replace(ph, nm=max(int(round(ph.nm * drug.nm_mod)), 1),
                   nc=max(int(round(ph.nc * drug.nc_mod)), 5),
                   laminAC=ph.laminAC * drug.laminAC_mod)
    f_treated = hepatocyte_function(E * drug.E_mod, ph_t, use_fast=False)
