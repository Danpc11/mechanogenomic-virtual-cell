...
temporal time constant, and the nuclear-response phenotype parameters
(motor-clutch parameters are held fixed unless supplied).
...
def fit_phenotype(data, name="fitted", base=None, reps=6):
    """Estimate the NUCLEAR-RESPONSE phenotype parameters from a full
    {(E, t): areas} dataset, keeping the motor-clutch parameters fixed.

    Estimates: laminAC, A_min, A_max, s0, tau.
    Held fixed (from `base`): nm, nc, kc, alpha, kon, koff0, Fm, vu, Fb.
    To also estimate motor parameters (nc, alpha, kc), use inference.py
    (ABC-SMC over nc, laminAC) or extend this routine with an optimizer.

    Steps: (1) deconvolve two populations per condition; (2) fit lamin, A_min,
    A_max from the mechanosensitive steady-state area vs stiffness (latest time
    point per stiffness as the steady-state estimate); (3) fit tau from the
    mechanosensitive temporal trajectory. Returns (Phenotype, report dict)."""
    if base is None:
